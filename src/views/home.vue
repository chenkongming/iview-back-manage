      ...
      <Row type="flex">
        <i-col :span="spanLeft" class="layout-menu-left">
          <Menu :active-name="0" width="auto" :open-names="[0]" v-show="spanLeft > 1" :accordion="true">
            <!--未收缩-->
            <Submenu v-for="(item,index) in secondMenuList" :name="index" :key="index" v-if="true">
              <template slot="title">
            ...
            <Submenu v-for="(item,index) in firstMenuList" :name="index" :key="index" v-if="false">
              <template slot="title">
                <Icon :type="menuIconMap[index]" size="15"></Icon>
                <span>{{item.title}}</span>
              </template>
              <Menu-item v-for="(secondItem,secondIndex) in item.secondMenu" :key="secondIndex" :name="index-secondIndex">
                {{secondItem.title}}
              </Menu-item>
              <Submenu v-if="item.secondMenu.length>0" v-for="(secondItem,secondIndex) in item.secondMenu"
                       :key="secondIndex" :name="index-secondIndex">
                <template slot="title">
                  <span>{{secondItem.title}}</span>
                </template>
                <Menu-item v-for="(thirdItem,thirdIndex) in secondItem.thirdMenu" :key="thirdIndex"
                           :name="index-secondIndex-thirdIndex">
                  {{thirdItem.title}}
                </Menu-item>
              </Submenu>
            </Submenu>
